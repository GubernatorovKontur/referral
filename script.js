// Глобальные переменные
 ...
 function displayStage(stageId) {
   ...
   const serviceName = "[Название сервиса]"; // Fallback, так как нет поля для сервиса
 
   // Заменяем плейсхолдеры в тексте этапа
   let stageText = stage.text;
   stageText = stageText.replaceAll("[Имя ЛПР]", lprName)
                       .replaceAll("[Имя]", lprName)
                       .replaceAll("[Название организации]", companyName)
                       .replaceAll("[Название сервиса]", serviceName);
 ...
 document.getElementById("copy-email-btn").onclick = () => {
   ...
   let emailText = `Добрый день, ${lprName}!
 ...
 Надеюсь на взаимовыгодное сотрудничество!`;
   ...
   // Заменяем все вхождения [Имя ЛПР] и [Название организации]
   emailText = emailText.replaceAll("[Имя ЛПР]", lprName).replaceAll("[Название организации]", companyName);
 
   // Копирование текста с использованием метода, аналогичного копированию лога
